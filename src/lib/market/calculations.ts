...
export function balances(state: TabMarketState) {
...
export function totalGroupVolume(state: TabMarketState) {
...
export function totalPicks(state: TabMarketState, person: string) {
...
export function peopleWithBetStats(state: TabMarketState): string[] {
...
export function accuracyPct(state: TabMarketState, person: string) {
  const st = state.stats[person] || { correct: 0, wrong: 0 };
  const n = totalPicks(state, person);
  return n ? (st.correct || 0) / n : 0;
}

export function alphaPct(state: TabMarketState, person: string) {
  const st = state.stats[person] || { alphaSum: 0, alphaCount: 0 };
  return st.alphaCount ? st.alphaSum / st.alphaCount : 0;
}

export function brierScore(state: TabMarketState, person: string) {
  const st = state.stats[person] || { brierSum: 0 };
  const n = totalPicks(state, person);
  return n && st.brierSum !== undefined ? st.brierSum / n : 0.25;
}

export function repScore(state: TabMarketState, person: string) {
  return Math.round(state.stats[person]?.elo || 1000);
}

export function rankForScore(state: TabMarketState, score: number) {
...
export function predictorScore(state: TabMarketState, person: string) {
  const n = totalPicks(state, person);
...
export function verdictCounts(state: TabMarketState, person: string) {
...
export function verdictLabel(state: TabMarketState, person: string) {
...
export function verdictSummary(state: TabMarketState, person: string) {
  const c = verdictCounts(state, person);
  return `${state.verdictLabels.good}: ${c.good} · ${state.verdictLabels.bad}: ${c.bad}`;
}

export function tagCounts(state: TabMarketState) {
...
export function personPickHistory(state: TabMarketState, person: string): PersonPick[] {
...
export function consistencyScore(state: TabMarketState, person: string): number | null {
...
export function improvementDelta(state: TabMarketState, person: string): number | null {
  const picks = personPickHistory(state, person);
...
export function predictionStreak(state: TabMarketState, person: string): number {
  const picks = personPickHistory(state, person);
  let streak = 0;
...
export function hotStreakLeaders(state: TabMarketState, min = HOT_STREAK_MIN) {
...
export function calibrationGrade(state: TabMarketState, person: string) {
...
export function rankingScore(state: TabMarketState, person: string, mode = "overall") {
...
export function comparePeople(state: TabMarketState, mode = "overall") {
...
export function personDebtBreakdown(state: TabMarketState, person: string) {
